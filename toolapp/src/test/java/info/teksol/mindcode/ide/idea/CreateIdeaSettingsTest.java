...
public class CreateIdeaSettingsTest {
    ...
    private final List<String> builtins = MindustryMetadata.getLatest().getAllLVars().stream()
            .map(LVariable::name).filter(name -> name.startsWith("@")).distinct().sorted().toList();
