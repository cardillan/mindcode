...
For a more comprehensive description of current custom function support, see [Custom Functions](SYNTAX-4-FUNCTIONS.markdown#user-defined-functions).
...
Operator precedence was changed to be more in line with usual implementations in other languages (specifically, Ruby).
The list of operators in order of precedence can be found [here](SYNTAX-2-EXPRESSIONS.markdown#operators).
...
The `println()` and `printf()` functions for easier printing (see [Built-in functions](SYNTAX-4-FUNCTIONS.markdown#built-in-functions)).
...
Most of Mindcode operators and deterministic built-in functions can be evaluated like this.
...
You might use parenthesis around constant expressions to isolate them, helping compile-time evaluation, e.g. `print(ticks * (60 / 1000))`.
