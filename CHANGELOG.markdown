...
* The default target has been changed from `8.1` to `8.2`, corresponding to the **v8 Build 160** Mindustry release.
* **Breaking:** a backslash character (`\`) in a string literal is now always treated as an escape character, in all targets. When updating to version 3.19.0, string literals containing backslashes need to be reviewed and updated.
* **Breaking:** The [`encode()` function](/doc/syntax/SYNTAX-4-FUNCTIONS.markdown#the-encode-function) in target `8.1` or lower has been restricted and no longer supports encoding value `92` (backslash) at all. In target `8.2` or higher, the `encode()` function supports all values in range `0 ... 65536`.  
* Compiler-generated strings (text jump tables, value translations, and strings generated by the `encode()` function) support all existing characters in target `8.2`.
...
* Added support for declaring arrays as local variables in functions, including [recursive ones](/doc/syntax/SYNTAX-4-FUNCTIONS.markdown#arrays-in-recursive-functions).
...
* Added the [`>>>` (unsigned right shift) operator](/doc/syntax/SYNTAX-2-EXPRESSIONS.markdown#shift-operators) to the compiler and processor emulator. Only available for target `8.0` or higher.
...
* **Breaking:** The system library was changed to accommodate the new logic instructions:
  ...
  * The `signExact` function in the `math` library was renamed to `sign`. This function corresponds to the Mindustry 8 instruction `op sign`, and when target `8.0` or higher is selected, the instruction is used instead of the library implementation.
  * The order of parameters of the `unpackcolor` function in the `graphics` library was changed to match the `unpackcolor` instruction in Mindustry 8. When target `8.0` or higher is selected, the instruction is used instead of the library implementation.
...
* Support for named color literals (e.g., `%[red]`) in target `8.0`.
