...
|`found = ulocate(:building, group, enemy, out outx, out outy, out building)`<br/>`group` - one of `:battery`, `:core`, `:factory`, `:generator`, `:reactor`, `:repair`, `:storage`, `:turret`.|`ulocate building group enemy @copper outx outy found building`|
