...
* `BLOCK_CHAR`
* `BLOCK_CHARACTER_OVERLAY`
* `BLOCK_CHARACTER_OVERLAY_WHITE`
...
* `BLOCK_METAL_TILES_1`
* `BLOCK_METAL_TILES_10`
* `BLOCK_METAL_TILES_11`
* `BLOCK_METAL_TILES_12`
* `BLOCK_METAL_TILES_13`
* `BLOCK_METAL_TILES_2`
* `BLOCK_METAL_TILES_3`
* `BLOCK_METAL_TILES_4`
* `BLOCK_METAL_TILES_5`
...
* `UNIT_RISSO`
* `UNIT_SCATHE_MISSILE`
* `UNIT_SCATHE_MISSILE_PHASE`
* `UNIT_SCATHE_MISSILE_SURGE`
* `UNIT_SCATHE_MISSILE_SURGE_SPLIT`
* `UNIT_SCEPTER`
* `UNIT_SEI`
* `UNIT_SPIROCT`
* `UNIT_STELL`
...
When Mindcode is compiled, the symbolic link names without default literal link names are resolved into literal link names, based on the type of the linked block declared in the `linked` modifier. The names are generated using the base block name and an index starting at 1. When a literal block link name is used in some linked variable declaration, symbolic link name resolution avoids the literal block link name, regardless of the declaration order.

> [!TIP]
> Assigning a literal link name to the symbolic link name allows producing mlog code matching a specific processor's links used in the game without having to make incompatible changes to the Mindcode source. When building a schematic, the link names are generated by the compiler, and the literal link names assigned by the user are ignored.  
...
When compiling Mindcode as part of Schemacode building, all symbolic link names are resolved. The compiler ignores literal link names assigned to symbolic link names, if present. The results of symbolic link name resolution are passed on to Schemacode, which uses them to build the schematic.
...
It is not possible to assign literal link names to symbolic link names in the declaration of a linked array.
