...
Returning from a recursive function requires several instructions (the exact amount depends on the type of stack used and the number of local arrays declared in the function). Returns from instructions are normally implemented as jumps to the return sequence at the end of the function. This optimization replaces these jumps with the full return sequence, saving one instruction execution at the price of increasing the code size by one instruction.
...
The tail call optimization can only be applied to simply recursive calls (i.e., a function calling itself). Function inlining can turn a group of mutually recursive functions into a simply recursive function, making it possible to apply the tail call optimization even in that case.
