...
* `void assertType(expectedType, actualValue, message)`: the function creates an assertion which compares the expected type with the runtime type of the actual value. The expected type is specified using one of the following mlog keywords:
  * `number`
  * `string`
  * `content`
  * `item`
