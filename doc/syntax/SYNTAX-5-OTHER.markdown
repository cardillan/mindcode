...
| [Case Switching](#case-switching)                                   | case-switching               |     N      |
| [Return Optimization](#return-optimization)                         | case-switching               |     N      |
| [Jump Straightening](#jump-straightening)                           | return-optimization          |     N      |
...
> **Note:** It is assumed that a case statement branching exclusively on integer values expects integer value on
> input as well. If the input value of the case expression may take on non-integer values, this optimization will
> produce wrong code. At this moment Mindcode isn't able to recognize such a situation; you need to disable the
> Case Switching optimization manually.
...
op add @counter <start_of_jump_table> offset
start_of_jump_table:
...
this limit, it is unusable, but if the compiled code is smaller than the limit, the unused instruction space bring no 
