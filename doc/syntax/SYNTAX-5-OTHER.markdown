...
* `speed` (the default value): Mindcode can generate additional instructions if it makes the resulting code faster while adhering to the 1000-instruction limit. When several possible optimizations of this kind are available, the ones having the best effect (the highest speedup per additional instruction generated) are selected until the instruction limit is reached.
