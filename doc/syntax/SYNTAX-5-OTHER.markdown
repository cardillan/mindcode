...
The limit only affects [dynamic optimizations](SYNTAX-6-OPTIMIZATIONS.markdown#static-and-dynamic-optimizations). The option has no effect on code generated by the compiler or static optimizations.
...
### Option `processor-size-limit`

**Option scope: [global](#global-scope)**

Sets the processor configuration size limit: the total size of the processor configuration (in bytes). The processor configuration contains compressed mlog code and compressed links configuration. When creating a schematic using Mindcode, the limit must not be exceeded both with the original mlog code and with the code as formatted after it is imported into the processor. Typically, the reformatted code is smaller than the original code (as labels, comments, and indentation are removed), but the code size may also increase due to the default values of omitted instruction arguments being filled out.

The default value of this option is 15,000 bytes. This is less than the actual limit of 16,000 bytes enforced by Mindustry to provide a safety margin for these reasons:
...
* `true`: instructions in the compiled code won't contain unused arguments. This usually makes the generated code more readable. When the code is imported into a processor, the missing arguments are replaced by default ones, which leads to a possible creation of additional, unused variables.
...
### Option `reformat-mlog`

**Option scope: [global](#global-scope)**

Activates/deactivates outputting a reformatted mlog code, i.e., the code as if it was inserted into a logic processor in-game and copied back. Possible values are:
...
* `false` (the default value when compiling Mindcode): compiler generates a warning when either limit is exceeded; emulator ignores the limits and processes all instructions normally.
* `true` (the default value when building a schematic): compiler generates an error when either limit is exceeded; emulator refuses to run the code.
