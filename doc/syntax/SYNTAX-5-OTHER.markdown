...
This option specifies how the compiler and emulator handle a situation where the instruction limit or code size limit has been exceeded. When inserting a code into a processor in Mindustry, the following two limits are enforced:
...
Possible values for the `enforce-instruction-limit` option are:
