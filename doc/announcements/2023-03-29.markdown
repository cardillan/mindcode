...
Check out [Function reference for Mindustry Logic 7](../syntax/FUNCTIONS_V7.markdown) for a list of all supported functions in Version 7.
...
[`ulocate(...)`](../syntax/FUNCTIONS_V7.markdown#instruction-ulocate) function).
...
The `println()` and `printf()` functions for easier printing (see [Built-in functions](../syntax/SYNTAX-4-FUNCTIONS.markdown#built-in-functions)).
