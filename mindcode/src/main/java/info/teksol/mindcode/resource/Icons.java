package info.teksol.mindcode.resource;
...
public class Icons {
    ...
    private static Map<String, LogicLiteral> initialize() {
        return ICONS_PROPERTIES.lines()
                .map(l -> l.split("[|=]"))
                .filter(s -> s.length == 3)
                .collect(Collectors.toMap(s -> s[2].toUpperCase().replaceAll("-UI$", ""),
                        s -> LogicString.create(((char) Integer.parseInt(s[0])) + "")));
    }

    // Compiled-in for now. Might be load from actual properties later on.
    private static final String ICONS_PROPERTIES = """
    ...
    private static final Map<String, LogicLiteral> ICON_MAP = initialize();
}
