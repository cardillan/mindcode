...
class DeadCodeEliminatorTest extends AbstractOptimizerTest<DeadCodeEliminator> {
    ...
    void preventsEliminationOfUradarUsages() {
        assertCompilesTo(expectedMessages().add("List of uninitialized variables: MIN_TO_MAX, SHOOT."),
                """
    ...
    void completelyRemovesDeadCode() {
        assertCompilesTo(expectedMessages().add("List of unused variables: n."),
                """
                        n = 1;
                        n = 1;
                        """,
    ...
    void removesUnusedUlocate() {
        assertCompilesTo(expectedMessages().add("List of unused variables: a, b."),
                """
                        ulocate(ore, @surge-alloy, out x, out y);
    ...
    void generatesUnusedWarning() {
        assertGeneratesMessages(
                expectedMessages().add("List of unused variables: X."),
                """
                        X = 10;
                        """
        );
    ...
    void generatesUninitializedWarning() {
        assertGeneratesMessages(
                expectedMessages().add("List of uninitialized variables: X, Y."),
                """
                        print(X, Y);
                        """
        );
    ...
    void removesUnusedGlobalParameters() {
        assertCompilesTo(expectedMessages().add("List of unused variables: a."),
                """
                        param a = 1;
                        param b = 2;
