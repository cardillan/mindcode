...
class CodeGeneratorTest extends AbstractCodeGeneratorTest {
    ...
    void compilesRealLifeTest1() {
        assertCompilesTo("""
                ...
                createInstruction(GETLINK, tmp(0), ":n"),
                createInstruction(SET, ":reactor", tmp(0)),
                createInstruction(OP, "notEqual", tmp(1), ":reactor", "null"),
                createInstruction(JUMP, label(2), "equal", tmp(1), "false"),
                createInstruction(SENSOR, tmp(2), ":reactor", "@liquidCapacity"),
                createInstruction(OP, "greaterThan", tmp(3), tmp(2), "0"),
                createInstruction(JUMP, label(3), "equal", tmp(3), "false"),
                createInstruction(SENSOR, tmp(5), ":reactor", "@cryofluid"),
                createInstruction(SENSOR, tmp(6), ":reactor", "@liquidCapacity"),
                createInstruction(OP, "div", tmp(7), tmp(5), tmp(6)),
                createInstruction(SET, ":pct_avail", tmp(7)),
                createInstruction(SET, tmp(8), ":reactor"),
                createInstruction(OP, "greaterThanEq", tmp(10), ":pct_avail", "0.25"),
                createInstruction(CONTROL, "enabled", tmp(8), tmp(10)),
                createInstruction(SET, tmp(4), tmp(10)),
