...
class UnreachableCodeEliminatorTest extends AbstractOptimizerTest<UnreachableCodeEliminator> {
    ...
    void keepsUsedFunctions() {
        assertCompilesTo(
                ...
                createInstruction(SETADDR, ":testa*retaddr", label(4)),
                createInstruction(CALL, label(2), "*invalid", ":testa*retval"),
                createInstruction(LABEL, label(4)),
                createInstruction(SETADDR, ":testa*retaddr", label(8)),
                createInstruction(CALL, label(2), "*invalid", ":testa*retval"),
                createInstruction(LABEL, label(8)),
                createInstruction(SETADDR, ":testc*retaddr", label(11)),
                createInstruction(CALL, label(1), "*invalid", ":testc*retval"),
                createInstruction(LABEL, label(11)),
                createInstruction(SETADDR, ":testc*retaddr", label(12)),
                createInstruction(CALL, label(1), "*invalid", ":testc*retval"),
                ...
                createInstruction(RETURN, ":testc*retaddr"),
                createInstruction(LABEL, label(2)),
                createInstruction(PRINT, q("Start")),
                createInstruction(RETURN, ":testa*retaddr")
        );
    }
