    ...
    class Arrays {
        ...
        void handlesArrays() {
            assertCompilesTo("""
                    ...
                    createInstruction(SETADDR, ".a*wret", label(14)),
                    createInstruction(OP, "mul", tmp(6), ":i", "2"),
                    createInstruction(MULTICALL, label(8), tmp(6)),
                    ...
                    createInstruction(SETADDR, ".a*rret", label(15)),
                    createInstruction(OP, "mul", tmp(7), ":i", "2"),
                    createInstruction(MULTICALL, label(11), tmp(7)),
                    createInstruction(LABEL, label(15)),
    ...
    class ExitPoints {
        ...
        void handlesReturnStatementsWithConditionalExpressions() {
            assertCompilesTo("""
                            ...
                            println(fib(2));
                            """,
                    createInstruction(SET, ":fib.0:n", "2"),
                    createInstruction(SETADDR, ":fib.0*retaddr", label(1)),
                    createInstruction(CALL, label(0), "*invalid", ":fib.0*retval"),
                    createInstruction(LABEL, label(1)),
                    createInstruction(PRINT, ":fib.0*retval"),
                    createInstruction(PRINT, q("\n")),
                    createInstruction(END),
                    createInstruction(LABEL, label(0)),
                    createInstruction(SET, ":fib.0*retval", "1"),
                    createInstruction(JUMP, label(4), "greaterThanEq", ":fib.0:n", "0"),
                    createInstruction(SET, ":fib.0*retval", "0"),
                    createInstruction(LABEL, label(4)),
                    createInstruction(RETURN, ":fib.0*retaddr")
            );
        }
    }
    ...
    class FunctionCalls {
        ...
        void compilesRecursiveFibonacci() {
            assertCompilesTo("""
                    ...
                    createInstruction(CALLREC, "bank1", label(0), label(2), ":fib.0*retval"),
                    createInstruction(LABEL, label(2)),
                    createInstruction(PRINT, ":fib.0*retval"),
                    createInstruction(END),
                    createInstruction(LABEL, label(0)),
                    createInstruction(SET, ":fib.0*retval", ":fib.0:n"),
                    createInstruction(JUMP, label(5), "lessThan", ":fib.0:n", "2"),
                    createInstruction(PUSH, "bank1", ":fib.0:n"),
                    createInstruction(OP, "sub", ":fib.0:n", ":fib.0:n", "1"),
                    createInstruction(CALLREC, "bank1", label(0), label(6), ":fib.0*retval"),
                    createInstruction(LABEL, label(6)),
                    createInstruction(POP, "bank1", ":fib.0:n"),
                    createInstruction(SET, tmp(4), ":fib.0*retval"),
                    createInstruction(PUSH, "bank1", tmp(4)),
                    createInstruction(OP, "sub", ":fib.0:n", ":fib.0:n", "2"),
                    createInstruction(CALLREC, "bank1", label(0), label(7), ":fib.0*retval"),
                    createInstruction(LABEL, label(7)),
        ...
        public void handlesConstantFunctionReturn() {
            assertCompilesTo("""
                    ...
                    createInstruction(SET, ":foo.0*retval", "5"),
        ...
        void handlesFunctionArgumentSetup() {
            assertCompilesTo("""
                    ...
                    createInstruction(LABEL, label(7)),
                    createInstruction(OP, "idiv", ":getBit.0:bitIndex", ":n", "2"),
                    createInstruction(SETADDR, ":getBit.0*retaddr", label(4)),
                    createInstruction(CALL, label(0), "*invalid", ":getBit.0*retval"),
                    createInstruction(LABEL, label(4)),
                    createInstruction(PRINT, ":getBit.0*retval"),
                    createInstruction(OP, "add", ":n", ":n", "1"),
                    createInstruction(JUMP, label(7), "lessThanEq", ":n", "1000"),
                    createInstruction(SET, ":getBit.0:bitIndex", "0"),
                    createInstruction(SETADDR, ":getBit.0*retaddr", label(5)),
                    createInstruction(CALL, label(0), "*invalid", ":getBit.0*retval"),
                    createInstruction(LABEL, label(5)),
                    createInstruction(END),
                    createInstruction(LABEL, label(0)),
                    createInstruction(OP, "mul", ":getBit.0*retval", ":getBit.0:bitIndex", "2"),
                    createInstruction(RETURN, ":getBit.0*retaddr")
            );
        }
        ...
        public void handlesInputOutputRecursiveFunctionParameters() {
            ...
            assertCompilesTo("""
                    ...
                    createInstruction(SET, ":foo.0:a", "1"),
                    createInstruction(SET, ":foo.0:b", "2"),
                    createInstruction(CALLREC, "bank1", label(0), label(2), ":foo.0*retval"),
                    createInstruction(LABEL, label(2)),
                    createInstruction(END),
                    createInstruction(LABEL, label(0)),
                    createInstruction(JUMP, label(4), "notEqual", ":foo.0:n", "0"),
                    createInstruction(SET, ":foo.0:a", "5"),
                    createInstruction(SET, ":foo.0:b", "10"),
                    createInstruction(RETURNREC, "bank1"),
                    createInstruction(LABEL, label(4)),
                    createInstruction(SET, tmp(5), ":foo.0:a"),
                    createInstruction(OP, "sub", ":foo.0:n", ":foo.0:n", "1"),
                    createInstruction(SET, ":foo.0:a", ":foo.0:b"),
                    createInstruction(SET, ":foo.0:b", tmp(5)),
                    createInstruction(CALLREC, "bank1", label(0), label(6), ":foo.0*retval"),
                    createInstruction(LABEL, label(6)),
                    createInstruction(SET, tmp(7), ":foo.0:b"),
                    createInstruction(SET, ":foo.0:b", ":foo.0:a"),
                    createInstruction(SET, ":foo.0:a", tmp(7)),
                    createInstruction(RETURNREC, "bank1")
            );
        }
        ...
        public void handlesInputRecursiveFunctionParameters() {
            ...
            assertCompilesTo("""
                    ...
                    createInstruction(SET, ":foo.0:a", "1"),
                    createInstruction(SET, ":foo.0:b", "2"),
                    createInstruction(SET, ":foo.0:c", "3"),
                    createInstruction(SET, ":foo.0:d", "4"),
                    createInstruction(CALLREC, "bank1", label(0), label(2), ":foo.0*retval"),
                    createInstruction(LABEL, label(2)),
                    createInstruction(END),
                    createInstruction(LABEL, label(0)),
                    createInstruction(JUMP, label(4), "notEqual", ":foo.0:n", "0"),
                    createInstruction(PRINT, ":foo.0:a"),
                    createInstruction(PRINT, ":foo.0:b"),
                    createInstruction(PRINT, ":foo.0:c"),
                    createInstruction(PRINT, ":foo.0:d"),
                    createInstruction(RETURNREC, "bank1"),
                    createInstruction(LABEL, label(4)),
                    createInstruction(SET, tmp(7), ":foo.0:a"),
                    createInstruction(OP, "sub", ":foo.0:n", ":foo.0:n", "1"),
                    createInstruction(SET, ":foo.0:a", ":foo.0:b"),
                    createInstruction(SET, ":foo.0:b", ":foo.0:c"),
                    createInstruction(SET, ":foo.0:c", ":foo.0:d"),
                    createInstruction(SET, ":foo.0:d", tmp(7)),
                    createInstruction(CALLREC, "bank1", label(0), label(6), ":foo.0*retval"),
                    createInstruction(LABEL, label(6)),
                    createInstruction(RETURNREC, "bank1")
            );
        ...
        void optimizesFunctionArguments() {
            assertCompilesTo("""
                    ...
                    createInstruction(END),
                    createInstruction(LABEL, label(0)),
                    createInstruction(OP, "mul", tmp(2), ":foo.0:n", "2"),
        ...
        public void handlesGlobalVariablesWithFunctionCalls() {
            assertCompilesTo("""
                            ...
                            bar(Y);
                            """,
                    createInstruction(OP, "rand", ":foo.0:n", "1000"),
                    createInstruction(OP, "rand", ".Y", "1000"),
                    createInstruction(SETADDR, ":foo.0*retaddr", label(1)),
                    createInstruction(CALL, label(0), "*invalid", ":foo.0*retval"),
                    createInstruction(LABEL, label(1)),
                    createInstruction(SET, ":foo.0:n", ".Y"),
    ...
    class InlineFunctions {
        ...
        void handlesComplexInlineFunctionArgumentSetup() {
            assertCompilesTo("""
                            ...
                            """,
                    createInstruction(PRINT, q("\n[green]Silicon[] status:\n  dome:  [green]{0}[]\n")),
                    createInstruction(SENSOR, ":printDomeStatus.0:level", "dome1", "@silicon"),
                    createInstruction(FORMAT, ":printDomeStatus.0:level")
            );
        }
