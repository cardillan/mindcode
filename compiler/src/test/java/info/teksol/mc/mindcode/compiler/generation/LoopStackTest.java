...
public class LoopStackTest {
    ...
    private final AstIdentifier
            ...
            loop = new AstIdentifier(EMPTY, "loop");
    ...
    private final AstBreakStatement breakLoopNode = new AstBreakStatement(EMPTY, loop);
    ...
    private final AstContinueStatement continueLoopNode = new AstContinueStatement(EMPTY, loop);

    @Nested
    class EmptyLabelsTest {
        @Test
        void remembersLabels() {
            loopStack.enterLoop(null, "loop", break1, continue1);
        ...
        void remembersMultipleLabels() {
            loopStack.enterLoop(null, "loop", break1, continue1);
            LoopStack.LoopLabels loopLabels = loopStack.enterLoop(null, "loop", break2, continue2);
        ...
        void rejectsProvidingLabelsOnEmptyStack() {
            ExpectedMessages.create()
                    .add("'break' statement outside of a do/while/for loop or a labeled code block.")
        ...
        void rejectsOutOfOrderLoopExits() {
            LoopStack.LoopLabels loopLabels1 = loopStack.enterLoop(label1, "loop", break1, continue1);
            LoopStack.LoopLabels loopLabels2 = loopStack.enterLoop(label2, "loop", break2, continue2);
    ...
    class NonEmptyLabelsTest {
        @Test
        void findsLabeledDefaultLabels() {
            loopStack.enterLoop(null, "loop", break1, continue1);
            loopStack.enterLoop(label2, "loop", break2, continue2);

            assertEquals(break1, loopStack.getBreakLabel(breakLoopNode));
            assertEquals(continue1, loopStack.getContinueLabel(continueLoopNode));
        ...
        void findsLabeledLabels() {
            loopStack.enterLoop(label1, "loop", break1, continue1);
            loopStack.enterLoop(label2, "loop", break2, continue2);
        ...
        void rejectsDuplicateLabels() {
            ExpectedMessages.create()
                    ...
                    .validate(consumer -> {
                        LoopStack loopStack = new LoopStack(consumer);
                        loopStack.enterLoop(label1, "loop", break1, continue1);
                        loopStack.enterLoop(label1, "loop", break2, continue2);
                    });
        }
