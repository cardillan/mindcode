...
public class BuiltinFunctionMlogBuilder extends AbstractFunctionBuilder {
    ...
    public ValueStore handleMlog(AstFunctionCall call, boolean safe) {
        ...
        if (args.getFirst().getArgumentValue() instanceof LogicString str) {
            ...
            error(args.getFirst().inputPosition(), "First argument to the '%s' function must be a string literal.",
        ...
        for (FunctionArgument arg : args.subList(1, args.size())) {
            if (arg.getArgumentValue() == LogicVariable.INVALID && !(arg instanceof MissingFunctionArgument)) {
                ...
                if (arg.hasOutModifier()) {
                    error(arg.inputPosition(), "A string literal passed to the '%s' function must not use an 'out' modifier.", call.getFunctionName());
                }
                ...
                if (arg.hasOutModifier() || arg.hasInModifier()) {
                    error(arg.inputPosition(), "A numeric literal passed to the '%s' function must not use any modifier.", call.getFunctionName());
                }
                arguments.add(lit);
                ...
                if (!value.isUserVariable()) {
                    error(arg.inputPosition(), "All arguments to the '%s' function must be literals or user variables.", call.getFunctionName());
                } else {
                    if (!arg.hasInModifier() && !arg.hasOutModifier()) {
                        error(arg.inputPosition(), "A variable passed to the '%s' function must use the 'in' and/or 'out' modifiers.", call.getFunctionName());
                    }
                    arguments.add(value);
                    if (arg.hasOutModifier()) {
