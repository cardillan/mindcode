...
public class AstBuilder extends MindcodeParserBaseVisitor<AstMindcodeNode> {
    ...
    private List<AstMindcodeNode> processBody(MindcodeParser.StatementListContext ctx) {
        return ctx != null ? ctx.expression().stream().map(this::visit).toList() : List.of();
    }
    ...
    @Override
    public AstMindcodeNode visitExpIfExpression(MindcodeParser.ExpIfExpressionContext ctx) {
        return new AstIfExpression(pos(ctx),
                new AstIfBranch(pos(ctx),visit(ctx.condition), processBody(ctx.trueBranch)),
    ...
    @Override
    public AstMindcodeNode visitExpForRangeLoop(MindcodeParser.ExpForRangeLoopContext ctx) {
        return new AstRangedForLoopStatement(pos(ctx),
                ctx.label != null ? createIdentifier(ctx.label) : null,
