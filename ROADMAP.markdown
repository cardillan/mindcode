...
* Variable type and invariant inferring.
  ...
  * Constraints on relationships between variables
    ...
    * e.g. `op max result input 10` establishes `max <= input`
...
* Complex loop unrolling
  ...
  * Jump-to-middle loop unrolling
    ...
    * Loops with fixed start that can run backwards (e.g. `for i in 0 .. n cell1[i] = 0 end` - jump to position
      ...
      * Still need a limit on highest possible value of `n`
  * Loop unswitching (if in loop --> loops in if)
  * Loop fusion???

### Further function optimizations
...
* If there are several combinations of argument values, each used more than once, it might make sense to create a
  ...
  it.
* When a read-only parameter is always passed the same argument value (a literal) and globally replace the parameter 
  with that literal, saving the assignment.
...
* Implement recursive calls by storing function return variable on stack like other variables, instead of pushing it 
  ...
  * Pro: it would be possible to eliminate callred and return instruction, making stackless and recursive calls more 
    similar.
